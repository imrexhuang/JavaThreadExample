...
import java.util.concurrent.Semaphore; //使用訊號量實現執行緒同步
...
public class SynchronizedBankFrame extends JFrame {
     ...
     *  第139行semaphore.acquire()和第143行semaphore.release()，執行後結果是540元
     */
    ...
    private class Transfer implements Runnable {
        ...
        public Transfer(Bank bank, Semaphore semaphore, JTextArea textArea) {// 初始化變數
            this.bank = bank;
        ...
        public void run() {
            for (int i = 0; i < 20; i++) {// 循環20次向賬戶存錢
                try {
                    semaphore.acquire();// 獲得一個許可
                    bank.deposit(11);// 向賬戶存入11塊錢
